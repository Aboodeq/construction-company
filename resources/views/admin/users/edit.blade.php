...
<x-admin.layouts.app title="تعديل مستخدم">
    <div class="mx-auto w-full max-w-2xl">
        <div class="mb-8">
            ...
            <h2 class="mt-4 font-display text-2xl font-semibold text-ink">{{ $user->name }}</h2>
            <p class="mt-2 max-w-2xl text-sm leading-6 text-ink-soft">{{ $user->email }}</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.users.update', $user) }}"
            ...
            <div class="flex items-center justify-between border-t border-line pt-6">
                <p class="text-xs text-ink-soft">
                    ...
                    آخر دخول: {{ $user->last_login_at?->diffForHumans() ?? 'لم يسجّل دخول بعد' }}
                </p>
