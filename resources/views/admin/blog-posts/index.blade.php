...
<x-admin.layouts.app title="المدونة">
    <div class="mx-auto w-full max-w-7xl">
        <div class="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div class="min-w-0">
                <h2 class="font-display text-2xl font-semibold text-ink">المدونة</h2>
                ...
                <p class="mt-3 max-w-3xl text-sm leading-6 text-ink-soft">
                    مقالات المدونة المنشورة بالموقع، وحالتها ومؤلفها وعدد مشاهداتها.
                </p>
            </div>

            <div class="flex items-center gap-3">
                <div class="rounded-lg border border-line bg-surface px-4 py-3 text-sm text-ink-soft">
                    <span class="font-medium text-ink">{{ $posts->total() }}</span>
                    مقالة ضمن النتائج الحالية
                </div>
                @can('blog.create')
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div class="rounded-lg border border-line bg-surface p-5">
                <p class="text-xs font-medium text-ink-soft">إجمالي المقالات</p>
            ...
            <div class="rounded-lg border border-line bg-surface p-5">
                <p class="text-xs font-medium text-ink-soft">المنشورة</p>
                <div class="mt-4 flex items-end justify-between gap-3">
            ...
            <div class="rounded-lg border border-line bg-surface p-5">
                <p class="text-xs font-medium text-ink-soft">إجمالي المشاهدات</p>
                <div class="mt-4 flex items-end justify-between gap-3">
                    <p class="font-display text-3xl font-bold text-brass">{{ number_format($stats['views']) }}</p>
        ...
        <form method="GET" action="{{ route('admin.blog.index') }}"
            ...
            <div class="grid grid-cols-1 gap-4 lg:grid-cols-12 lg:items-end">
                <div class="lg:col-span-5">
                    <label for="blog-search" class="mb-2 block text-xs font-medium text-ink-soft">بحث</label>
                    <div class="relative">
                        <input id="blog-search" name="search" value="{{ request('search') }}" type="search"
                ...
                <div class="lg:col-span-3">
                    <label for="blog-category" class="mb-2 block text-xs font-medium text-ink-soft">التصنيف</label>
                ...
                <div class="lg:col-span-2">
                    <label for="blog-status" class="mb-2 block text-xs font-medium text-ink-soft">الحالة</label>
        ...
        <section class="mt-6 overflow-hidden rounded-lg border border-line bg-surface">
            <div class="flex flex-col gap-3 border-b border-line px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h3 class="font-display text-lg font-semibold text-ink">قائمة المقالات</h3>
                    <p class="mt-1 text-sm text-ink-soft">أحدث المقالات أولًا حسب تاريخ آخر تعديل.</p>
                </div>
                <span class="text-sm text-ink-soft">
                    {{ $posts->firstItem() ?? 0 }} - {{ $posts->lastItem() ?? 0 }} من {{ $posts->total() }}
                </span>
            </div>
            ...
            <div class="divide-y divide-line">
                        ...
                        <div class="grid grid-cols-1 gap-5 xl:grid-cols-12 xl:items-center">
                            <div class="min-w-0 xl:col-span-5">
                                <div class="flex min-w-0 gap-4">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        ...
                                        @else
                                            <h4 class="font-semibold text-ink">{{ $post->title }}</h4>
                                        @endcan
                                        <p class="mt-1 truncate text-xs text-ink-soft">
                                            {{ $post->author?->name ?? 'بدون كاتب' }}
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4 text-sm xl:col-span-4">
                                <div>
                                    ...
                                    <p class="mt-1 text-ink">{{ $post->category?->name ?? 'بدون تصنيف' }}</p>
                                </div>
                                <div>
                                    <p class="text-xs font-medium text-ink-soft">الحالة</p>
                                    <span
                                        class="mt-1 inline-flex rounded-full border px-2.5 py-1 text-xs font-medium {{ $statusClasses[$post->status] ?? 'border-line bg-paper text-ink-soft' }}">
                                        {{ $statusLabels[$post->status] ?? $post->status }}
                                    </span>
                                </div>
                                <div>
                                    <p class="text-xs font-medium text-ink-soft">المشاهدات</p>
                                    <p class="mt-1 font-semibold text-ink">{{ number_format($post->views_count) }}</p>
                                </div>
                                <div>
                                    <p class="text-xs font-medium text-ink-soft">تاريخ النشر</p>
                                    <p class="mt-1 text-ink">{{ $post->published_at?->format('Y-m-d') ?? '—' }}</p>
                                </div>
                            </div>
                ...
                @empty
                    <div class="px-5 py-14 text-center">
                        <p class="font-medium text-ink">لا توجد مقالات مطابقة</p>
