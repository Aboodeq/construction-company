<x-admin.layouts.app title="الإحصائيات">
    <div class="mx-auto w-full max-w-4xl">
        <div class="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div class="min-w-0">
                <h2 class="font-display text-2xl font-semibold text-ink">إحصائيات الشركة</h2>
                ...
                <p class="mt-3 max-w-3xl text-sm leading-6 text-ink-soft">
                    الأرقام والإنجازات المعروضة في قسم الإحصائيات بالصفحة الرئيسية.
                </p>
            </div>

            <a href="{{ route('admin.company-statistics.create') }}"
            ...
            <div class="divide-y divide-line">
                @forelse ($statistics as $statistic)
                    <article class="flex flex-wrap items-center gap-4 p-5 transition hover:bg-paper/45">
                        ...
                        <div class="min-w-0 flex-1">
                            <a href="{{ route('admin.company-statistics.edit', $statistic) }}"
                                class="font-display text-xl font-bold text-ink transition hover:text-brass">
                                {{ number_format($statistic->number) }}{{ $statistic->suffix }}
                            </a>
                            <p class="mt-1 text-sm text-ink-soft">{{ $statistic->label }}</p>
                        </div>
                        ...
                        <div class="flex shrink-0 items-center gap-2">
                            ...
                            <x-admin.confirm-form
                                :action="route('admin.company-statistics.destroy', $statistic)"
                                title="حذف الإحصائية"
                                :message="'سيتم حذف إحصائية «'.$statistic->label.'» نهائيًا. هل تريد المتابعة؟'"
                                class="h-9 w-9"
                                triggerLabel="حذف">
                ...
                @empty
                    <div class="px-5 py-14 text-center">
                        <p class="font-medium text-ink">لا توجد إحصائيات بعد</p>
                        <p class="mt-2 text-sm text-ink-soft">أضف أول إحصائية لتظهر بالصفحة الرئيسية.</p>
                    </div>
                @endforelse
            </div>
        </section>
        ...
        @if ($statistics->hasPages())
            <div class="mt-6 flex justify-center gap-2 text-sm text-ink-soft">
                ...
                @else
                    <a href="{{ $statistics->previousPageUrl() }}" class="rounded-lg border border-line px-4 py-2 hover:bg-surface hover:text-ink">السابق</a>
                @endif
                @if ($statistics->hasMorePages())
                    <a href="{{ $statistics->nextPageUrl() }}" class="rounded-lg border border-line px-4 py-2 hover:bg-surface hover:text-ink">التالي</a>
                @else
