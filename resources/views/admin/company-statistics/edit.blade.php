<x-admin.layouts.app title="تعديل إحصائية">
    <div class="mx-auto w-full max-w-2xl">
        <div class="mb-8">
            ...
            <h2 class="mt-4 font-display text-2xl font-semibold text-ink">تعديل إحصائية</h2>
        </div>

        <form method="POST" action="{{ route('admin.company-statistics.update', $companyStatistic) }}"
            class="space-y-6 rounded-lg border border-line bg-surface p-6">
            @csrf
            @method('PUT')

            @include('admin.company-statistics.partials.fields', ['companyStatistic' => $companyStatistic])

            <div class="flex items-center justify-between gap-3 border-t border-line pt-6">
                <x-admin.confirm-form
                    :action="route('admin.company-statistics.destroy', $companyStatistic)"
                    title="حذف الإحصائية"
                    :message="'سيتم حذف إحصائية «'.$companyStatistic->label.'» نهائيًا. هل تريد المتابعة؟'"
