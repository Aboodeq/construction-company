<x-admin.layouts.app title="مراحل العمل">
    <div class="mx-auto w-full max-w-4xl">
        <div class="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div class="min-w-0">
                <h2 class="font-display text-2xl font-semibold text-ink">مراحل العمل</h2>
                ...
                <p class="mt-3 max-w-3xl text-sm leading-6 text-ink-soft">
                    خطوات آلية العمل المعروضة بالصفحة الرئيسية، من أول تواصل حتى التسليم.
                </p>
            </div>
            ...
            <div class="divide-y divide-line">
                @forelse ($processSteps as $processStep)
                    <article class="flex flex-wrap items-center gap-4 p-5 transition hover:bg-paper/45">
                        <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-brass-soft font-display text-sm font-bold text-brass">
                            {{ $processStep->step_number }}
                        </span>

                        <div class="min-w-0 flex-1">
                            <a href="{{ route('admin.process-steps.edit', $processStep) }}"
                                class="font-semibold text-ink transition hover:text-brass">
                                {{ $processStep->title }}
                            </a>
                            @if ($processStep->description)
                                <p class="mt-1 line-clamp-1 max-w-md text-sm text-ink-soft">{{ $processStep->description }}</p>
                            @endif
                        </div>

                        <span class="shrink-0 text-xs text-ink-soft">ترتيب: {{ $processStep->order }}</span>
                        ...
                        <div class="flex shrink-0 items-center gap-2">
                            ...
                            <x-admin.confirm-form
                                :action="route('admin.process-steps.destroy', $processStep)"
                                title="حذف الخطوة"
                                :message="'سيتم حذف مرحلة «'.$processStep->title.'» نهائيًا. هل تريد المتابعة؟'"
                                class="h-9 w-9"
                                triggerLabel="حذف">
                ...
                @empty
                    <div class="px-5 py-14 text-center">
                        <p class="font-medium text-ink">لا توجد مراحل بعد</p>
                        <p class="mt-2 text-sm text-ink-soft">أضف أول مرحلة لتظهر بالصفحة الرئيسية.</p>
                    </div>
                @endforelse
            </div>
        </section>
        ...
        @if ($processSteps->hasPages())
            <div class="mt-6 flex justify-center gap-2 text-sm text-ink-soft">
                @if ($processSteps->onFirstPage())
                    <span class="rounded-lg border border-line px-4 py-2 text-ink-soft/50">السابق</span>
                @else
                    <a href="{{ $processSteps->previousPageUrl() }}" class="rounded-lg border border-line px-4 py-2 hover:bg-surface hover:text-ink">السابق</a>
                @endif
                @if ($processSteps->hasMorePages())
                    <a href="{{ $processSteps->nextPageUrl() }}" class="rounded-lg border border-line px-4 py-2 hover:bg-surface hover:text-ink">التالي</a>
                @else
