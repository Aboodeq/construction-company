<x-admin.layouts.app title="إضافة دور">
    <div class="mx-auto w-full max-w-4xl">
        <div class="mb-8">
            <a href="{{ route('admin.roles.index') }}"
                ...
                العودة إلى الأدوار والصلاحيات
            </a>
            <h2 class="mt-4 font-display text-2xl font-semibold text-ink">إضافة دور جديد</h2>
            <p class="mt-2 max-w-2xl text-sm leading-6 text-ink-soft">
                اختر اسمًا للدور، ثم حدد الصلاحيات التي يملكها أي مستخدم يُسند إليه هذا الدور.
            </p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.roles.store') }}"
            ...
            <div class="max-w-sm">
                ...
                <x-text-input id="name" name="name" type="text" required autofocus
                    :value="old('name')" placeholder="مثال: مسؤول التسويق" />
                <x-input-error :messages="$errors->get('name')" />
            </div>

            <div>
                <p class="mb-3 text-sm font-medium text-ink">الصلاحيات</p>
                @include('admin.roles.partials.permissions', ['rolePermissions' => []])
            </div>
