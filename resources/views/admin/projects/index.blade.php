...
<x-admin.layouts.app title="المشاريع">
    <div class="mx-auto w-full max-w-7xl">
        <div class="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div class="min-w-0">
                <h2 class="font-display text-2xl font-semibold text-ink">المشاريع</h2>
                ...
                <p class="mt-3 max-w-3xl text-sm leading-6 text-ink-soft">
                    معرض أعمال الشركة المنجزة، بما في ذلك بيانات العميل والموقع والخدمات المستخدمة في كل مشروع.
                </p>
            </div>

            <div class="flex items-center gap-3">
                <div class="rounded-lg border border-line bg-surface px-4 py-3 text-sm text-ink-soft">
                    <span class="font-medium text-ink">{{ $projects->total() }}</span>
                    مشروع ضمن النتائج الحالية
                </div>
                @can('projects.create')
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div class="rounded-lg border border-line bg-surface p-5">
                <p class="text-xs font-medium text-ink-soft">إجمالي المشاريع</p>
            ...
            <div class="rounded-lg border border-line bg-surface p-5">
                <p class="text-xs font-medium text-ink-soft">المشاريع المنشورة</p>
                <div class="mt-4 flex items-end justify-between gap-3">
            ...
            <div class="rounded-lg border border-line bg-surface p-5">
                <p class="text-xs font-medium text-ink-soft">مشاريع مميزة</p>
                <div class="mt-4 flex items-end justify-between gap-3">
        ...
        <form method="GET" action="{{ route('admin.projects.index') }}"
            ...
            <div class="grid grid-cols-1 gap-4 lg:grid-cols-12 lg:items-end">
                <div class="lg:col-span-5">
                    <label for="projects-search" class="mb-2 block text-xs font-medium text-ink-soft">بحث</label>
                    <div class="relative">
                        <input id="projects-search" name="search" value="{{ request('search') }}" type="search"
                            placeholder="اسم المشروع أو العميل أو الموقع"
                ...
                <div class="lg:col-span-3">
                    <label for="projects-category" class="mb-2 block text-xs font-medium text-ink-soft">التصنيف</label>
                    <select id="projects-category" name="category"
                ...
                <div class="lg:col-span-2">
                    <label for="projects-status" class="mb-2 block text-xs font-medium text-ink-soft">الحالة</label>
                    <select id="projects-status" name="status"
                        class="h-11 w-full rounded-lg border-line bg-paper text-sm text-ink focus:border-brass focus:ring-brass">
        ...
        <section class="mt-6 overflow-hidden rounded-lg border border-line bg-surface">
            <div class="flex flex-col gap-3 border-b border-line px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h3 class="font-display text-lg font-semibold text-ink">قائمة المشاريع</h3>
                    <p class="mt-1 text-sm text-ink-soft">عرض مرتب لمعرض الأعمال مع بيانات العميل والموقع.</p>
                </div>
                <span class="text-sm text-ink-soft">
                    {{ $projects->firstItem() ?? 0 }} - {{ $projects->lastItem() ?? 0 }} من {{ $projects->total() }}
                </span>
            </div>
            ...
            <div class="divide-y divide-line">
                        ...
                        <div class="grid grid-cols-1 gap-5 xl:grid-cols-12 xl:items-center">
                            <div class="min-w-0 xl:col-span-4">
                                <div class="flex min-w-0 gap-4">
                                    @if ($project->cover_image)
                                        <img src="{{ asset('storage/' . ltrim($project->cover_image, '/')) }}"
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <div class="flex flex-wrap items-center gap-2">
                                            ...
                                            @else
                                                <h4 class="font-semibold text-ink">{{ $project->title }}</h4>
                                            @endcan
                                        ...
                                        <p class="mt-1 truncate text-xs text-ink-soft">
                                            {{ $project->client_name ?? 'بدون عميل محدد' }}
                                            @if ($project->location)
                                                · {{ $project->location }}
                                            @endif
                                        </p>
                            ...
                            <div class="grid grid-cols-2 gap-4 text-sm xl:col-span-3">
                                <div>
                                    ...
                                    <p class="mt-1 text-ink">{{ $project->category?->name ?? 'بدون تصنيف' }}</p>
                                </div>
                                <div>
                                    <p class="text-xs font-medium text-ink-soft">الحالة</p>
                                    <span
                                        class="mt-1 inline-flex rounded-full border px-2.5 py-1 text-xs font-medium {{ $statusClasses[$project->status] ?? 'border-line bg-paper text-ink-soft' }}">
                                        {{ $statusLabels[$project->status] ?? $project->status }}
                                    </span>
                                </div>
                                <div>
                                    <p class="text-xs font-medium text-ink-soft">المساحة</p>
                                    <p class="mt-1 text-ink">{{ $project->area ? number_format((float) $project->area) . ' م²' : '—' }}</p>
                                </div>
                                <div>
                                    <p class="text-xs font-medium text-ink-soft">آخر تحديث</p>
                                    <p class="mt-1 text-ink">{{ $project->updated_at?->format('Y-m-d') }}</p>
                                </div>
                            </div>
                            ...
                            <div class="xl:col-span-2">
                                ...
                                <div class="mt-3 flex flex-wrap gap-x-4 gap-y-2 text-sm text-ink">
                                    <span>{{ $project->images_count }} صور</span>
                                    <span>{{ $project->services_count }} خدمات</span>
                                    <span>{{ $project->testimonials_count }} آراء</span>
                                </div>
                            </div>
                ...
                @empty
                    <div class="px-5 py-14 text-center">
                        <p class="font-medium text-ink">لا توجد مشاريع مطابقة</p>
        ...
        @if ($projects->hasPages())
            <div class="mt-6 flex flex-col gap-3 border-t border-line pt-5 text-sm text-ink-soft sm:flex-row sm:items-center sm:justify-between">
                <p>
                    عرض {{ $projects->firstItem() }} - {{ $projects->lastItem() }} من {{ $projects->total() }}
                </p>
